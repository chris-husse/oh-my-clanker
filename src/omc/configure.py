...
def known_models(ctx: ToolContext, name: str) -> list[str]:
    """Picker suggestions, preferring the harness's own catalog.

    Static aliases are useful suggestions but need not list every valid ID.
    Validation must use only ``_catalog_models`` so full model IDs remain
    usable when a provider has no authoritative catalog.
    """
    return _catalog_models(ctx, name) or get_provider(name).models()


def _catalog_models(ctx: ToolContext, name: str) -> list[str]:
    """Model IDs from a successful catalog probe, or [] when unavailable.

    No catalog command, a missing binary, a non-zero exit, or unparseable
    output must leave free-text model configuration available.
    """
...
def _check_model(ctx: ToolContext, key: str, value: str) -> None:
    ...
    if parts[:2] != ["llm", "providers"] or parts[-1:] != ["model"] or not value:
        return
    name = parts[2]
    known = _catalog_models(ctx, name)
