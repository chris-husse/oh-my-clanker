...
def test_notify_cli_rejects_explicit_event_flags_for_supported_provider(tmp_path, monkeypatch):
    from omc.internal import run_internal

    monkeypatch.setenv("OMC_HOME", str(tmp_path / "empty-home"))
    assert run_internal(["notify", "--provider", "codex", "--event", "session.idle"]) == 2
...
def test_run_notify_codex_path(tmp_path):
    home = tmp_path / "home"
    log = tmp_path / "n.log"
    ...
    assert [ln[2:] for ln in lines] == [["codex", "agent-turn-complete", "turn complete"]]
...
class FileHookProvider:
    name = "fixture"

    def notification_setup(self, sink):
        return {"hooks/notify.js": "// generated by omc — fixture\n" + " ".join(sink) + "\n"}
...
def test_wire_worktree_writes_fresh_files(tmp_path):
    ...
    assert notify.wire_worktree(FileHookProvider(), tmp_path) == ["hooks/notify.js"]
    assert "omc internal notify --provider fixture" in (tmp_path / "hooks/notify.js").read_text()
...
def test_wire_worktree_leaves_foreign_file_alone(tmp_path, capsys):
    target = tmp_path / "hooks" / "notify.js"
    target.parent.mkdir(parents=True)
    target.write_text("// the user's own hook\n")
    assert notify.wire_worktree(FileHookProvider(), tmp_path) == []
    assert target.read_text() == "// the user's own hook\n"
    assert "leaving it alone" in capsys.readouterr().err


def test_wire_worktree_upgrades_own_stale_file(tmp_path):
    target = tmp_path / "hooks" / "notify.js"
    target.parent.mkdir(parents=True)
    target.write_text("// generated by omc — idle notifications (OLD template)\nold body\n")
    assert notify.wire_worktree(FileHookProvider(), tmp_path) == ["hooks/notify.js"]
    assert "generated by omc — fixture" in target.read_text()
