...
@pytest.mark.parametrize("model", ["claude-fable-5", "opus"])
def test_set_accepts_claude_full_model_ids_and_aliases(tmp_path, model):
    # Claude's static picker aliases are suggestions, not an exhaustive catalog.
    ctx = ToolContext.from_env(stub_env(tmp_path / "bin"))
    assert run_configure(ctx, defaults=False, sets=[f"llm.providers.claude.model={model}"]) == 0
    assert store.load_global(ctx.home).llm.providers["claude"].model == model


@pytest.mark.parametrize("stdout,rc", [(_CATALOG, 1), ("not json", 0), ('{"models": []}', 0)])
def test_set_accepts_model_when_catalog_is_unavailable(tmp_path, monkeypatch, stdout, rc):
    from omc.providers.registry import get_provider

    # Even if a provider has fallback suggestions, failed/empty catalog probes
    # must not turn those suggestions into validation evidence.
    monkeypatch.setattr(get_provider("codex"), "models", lambda: ["suggested-model"])
    ctx = _catalog_stub(tmp_path / "bin", stdout=stdout, rc=rc)
    model = "custom-model"
    assert run_configure(ctx, defaults=False, sets=[f"llm.providers.codex.model={model}"]) == 0
    assert store.load_global(ctx.home).llm.providers["codex"].model == model
