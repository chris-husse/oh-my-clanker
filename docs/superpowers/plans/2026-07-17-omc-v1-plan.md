...
**Goal:** Build the open-source `omc` CLI + multi-harness skills plugin: `omc start <context>` turns a ticket/description into a prepared worktree with a named, seeded LLM session (Claude Code / Codex), with Docker-per-test E2E.

**Architecture:** A uv-installed Python package (`src/omc/`) owns launch-time work (probe → slug via headless LLM → wt worktree → shell/terminal handoff). The same repo is a plugin for both harnesses (`.claude-plugin/`, `.codex-plugin/`) delivering `skills/slug` and `skills/start`. E2E runs each test in a fresh Docker container via testcontainers-python with a hermetic stub Jira MCP.
...
**Files:**
- Create: `src/omc/providers/__init__.py` (empty), `src/omc/providers/base.py`, `src/omc/providers/claude.py`, `src/omc/providers/codex.py`, `src/omc/providers/registry.py`
- Test: `tests/unit/test_providers.py`

**Interfaces:**
- Produces: `Provider` ABC with `name: str`, `models() -> list[str]`, `headless_argv(prompt, *, model, allowed_tools=None) -> list[str]`, `session_argv(*, session_name, model, seed) -> list[str]`, `title_env() -> dict[str, str]`, `install_hint() -> str`; `registry.get_provider(name) -> Provider` (raises `OmcError` on unknown), `registry.provider_names() -> list[str]` (== `["claude", "codex"]`).
...
```bash
git add -A && git commit -m "feat: claude/codex provider adapters"
```
...
def test_configure_set(tmp_path, monkeypatch):
    ...
    rc = main([
        "configure",
        "--set", "llm.default=codex",
        "--set", "llm.providers.codex.model=example-model",
...
auto-installs it as a dependency; on Codex install it from
...
**Files:**
- Create: `skills/start/SKILL.md`, `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.codex-plugin/plugin.json`
- Test: `tests/unit/test_plugin_manifests.py`

**Interfaces:**
- Produces: the session-side skill and installable plugin shapes for both harnesses. Claude plugin declares `"dependencies": ["superpowers"]`.
...
```bash
git add -A && git commit -m "feat: start skill + claude/codex plugin manifests"
```
...
**Interfaces:**
- Produces: image `omc-e2e` containing node 22 + python3 + uv + git + wt + claude/codex CLIs + this repo at `/repo` with `omc` on PATH + omc & superpowers plugins registered for Claude Code + stub MCP at `/repo/docker/stub-jira-mcp/`. Container default command keeps it alive (`sleep infinity`).
...
docker run --rm omc-e2e:dev sh -c "omc version && git --version && wt --version && claude --version && codex --version && echo IMAGE-OK"
```
...
```bash
git add -A && git commit -m "feat: E2E docker image with both provider CLIs + wt"
```
...
Run: `just e2e-tests tests/e2e/test_e2e_slug_matrix.py -k claude` (then codex as tokens allow; without a token the test FAILS with the export guidance — that is the designed behavior, not a bug).
...
1. Title "Oh My Clanker!" + one-liner: *Turn "I have a ticket" into "I'm in a prepared worktree with an LLM session that already knows the ticket" — for Claude Code and Codex.*
2. Install: `uv tool install git+https://github.com/chris-husse/oh-my-clanker`, then `omc configure`, then per-harness plugin install (the exact two-row table from `configure`'s hint text).
